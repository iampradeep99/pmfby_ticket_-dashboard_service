...
export class TicketDashboardService {
  ...
  // Agent info (only 1)
  {
    $lookup: {
      from: 'csc_agent_master',
      let: { userId: '$InsertUserID' },
      pipeline: [
        { $match: { $expr: { $eq: ['$UserLoginID', '$$userId'] } } },
        { $limit: 1 },
      ],
      as: 'agentInfo',
    },
  ...
  // Remove $group + $replaceRoot (already unique per SupportTicketNo)
  // Project fields
  {
    $project: {
      SupportTicketID: 1,
