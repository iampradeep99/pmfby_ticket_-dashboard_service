...
<p>Regards,<br/>Your Automation System</p>
  ...
  async sendFileToGCP(documentsPath: any, fileNameGCP: any, item:any) {
    ...
    formData.append("filePath", "krph_reports/October2025");
    formData.append("uploadedBy", "KRPH");
    formData.append("documents", documentsPath, { filename: fileNameGCP });
  ...
  getUTCStartEndForDateUTC(dateStr: string) {
    const [day, month, year] = dateStr.split("-").map(Number);
  ...
  async fileNameForGCP(url) {

    const base = "https://krph.csccloud.in/RECORDINGS/";
    const baseIndex = url.indexOf(base);

    if (baseIndex !== -1) {
      const remainingPath = url.slice(baseIndex + base.length);
  ...
  async getFileName(url) {
    const filename = await this.fileNameForGCP(url);
    return filename
  }



  async getAudioFile(string) {
    try {
  ...
  async CallingUrlApiCall(recordingId: string) {
    const url = `https://krph.csccloud.in/api/fetch_recording_url/${recordingId}`;
...
/* async SaveGCPFileToDB(payload) {
  try {
    let collection = "KRPH_Calling_CDR_files_paths";
    let record = await this.db.collection(collection).findOne({ uniqueId: payload?.uniqueId });
    ...
    if (record) {
      ...
      await this.db.collection(collection).insertOne(payload)

      let apiURL = "https://pmfby.gov.in/krphdashboard/ticket-dashboard/SaveCDRFilePath";
      const response = await axios.post(apiURL, payload);
      
      if (response.status === 200) {
        console.log('File successfully saved to DB:', response.data);
        return response.data;
      } else {
        console.error('Failed to save file to DB. Status:', response.status, 'Response:', response.data);
        throw new Error('Failed to save file to DB');
      }
    }

  } catch (err) {
    console.error('Error saving file to DB:', err.message || err);
    throw new Error('Error while saving file to DB: ' + (err.message || err));
  }
} */
  ...
  try {
    if (!payload?.uniqueId) {
      throw new Error("Payload must contain a uniqueId.");
    }
    ...
    if (record) {
      ...
      console.log("New record inserted with ID:", result.insertedId);
...
async isUniqueIdInDB(item) {
  try {
    let collection = "KRPH_Calling_CDR_files_paths";
    
    let data = await this.db.collection(collection).findOne({ uniqueId: item?.CallingUniqueID });
    
    return !!data; 
  } catch (err) {
    console.error('Error in isUniqueIdInDB:', err);
    return false; 
  }
}
