...
async processTicketHistoryAndGenerateZipdddd(ticketPayload: any) {
  let {
    SPFROMDATE,
    SPTODATE,
    SPInsuranceCompanyID,
...
async processTicketHistoryAndGenerateZipCSVWorking(ticketPayload: any) {
  ...
  if (SPInsuranceCompanyID && SPInsuranceCompanyID !== '#ALL') {
    ...
    const allowedInsuranceIDs = InsuranceCompanyID.map(Number);
    const validInsuranceIDs = requestedInsuranceIDs.filter(id => allowedInsuranceIDs.includes(id));
    if (!validInsuranceIDs.length) return { rcode: 0, rmessage: 'Unauthorized InsuranceCompanyID(s).' };
    baseMatch.InsuranceCompanyID = { $in: validInsuranceIDs };
  } else if (InsuranceCompanyID?.length) baseMatch.InsuranceCompanyID = { $in: InsuranceCompanyID.map(Number) };

  if (SPStateID && SPStateID !== '#ALL') {
    const requestedStateIDs = SPStateID.split(',').map(id => id.trim());
    const validStateIDs = requestedStateIDs.filter(id => StateMasterID.includes(id));
    if (!validStateIDs.length) return { rcode: 0, rmessage: 'Unauthorized StateID(s).' };
    baseMatch.FilterStateID = { $in: validStateIDs };
  } else if (StateMasterID?.length && LocationTypeID !== 2) baseMatch.FilterStateID = { $in: StateMasterID };

  // --- Prepare folder & CSV ---
  const folderPath = path.join(process.cwd(), 'downloads');
  await fs.promises.mkdir(folderPath, { recursive: true });

  const csvFileName = `support_ticket_data_${Date.now()}.csv`;
  const csvFilePath = path.join(folderPath, csvFileName);
  const writeStream = fs.createWriteStream(csvFilePath);
  ...
  async function processDateRecursive(currentDate: Date, endDate: Date) {
    ...
    const pipeline: any[] = [
      ...
      // {
      //   $project: {
      //     SupportTicketID: 1, ApplicationNo: 1, InsurancePolicyNo: 1, TicketStatusID: 1, TicketStatus: 1,
      //     CallerContactNumber: 1, RequestorName: 1, RequestorMobileNo: 1, StateMasterName: 1, DistrictMasterName: 1,
      //     SubDistrictName: 1, TicketHeadName: 1, TicketCategoryName: 1, RequestSeason: 1, RequestYear: 1,
      //     ApplicationCropName: 1, Relation: 1, RelativeName: 1, PolicyPremium: 1, PolicyArea: 1, PolicyType: 1,
      //     LandSurveyNumber: 1, LandDivisionNumber: 1, IsSos: 1, PlotStateName: 1, PlotDistrictName: 1,
      //     PlotVillageName: 1, ApplicationSource: 1, CropShare: 1, IFSCCode: 1, FarmerShare: 1, SowingDate: 1,
      //     LossDate: 1, CreatedBY: 1, CreatedAt: "$InsertDateTime", Sos: 1, NCIPDocketNo:"$TicketNCIPDocketNo",
      //     TicketDescription: 1, CallingUniqueID: 1,
      //     TicketDate: { $dateToString: { format: "%Y-%m-%d %H:%M:%S", date: "$Created" } },
      //     StatusDate: { $dateToString: { format: "%Y-%m-%d %H:%M:%S", date: "$StatusUpdateTime" } },
      //     SupportTicketTypeName: "$TicketTypeName", SupportTicketNo: 1, InsuranceMasterName: "$InsuranceCompany",
      //     ReOpenDate: "$TicketReOpenDate", CallingUserID: "$agentInfo.UserID", SchemeName: 1,
      //   }
      // }

      {
  $project: {
    "Agent ID": "$agentInfo.UserID",                        // CallingUserID
    "Calling ID": "$CallingUniqueID",
    "NCIP Docket No": "$TicketNCIPDocketNo",
    "Ticket No": "$SupportTicketNo",
    "Creation Date": { $dateToString: { format: "%Y-%m-%d %H:%M:%S", date: "$Created" } },
    "Re-Open Date": "$TicketReOpenDate",
    "Ticket Status": "$TicketStatus",
    "Status Date": { $dateToString: { format: "%Y-%m-%d %H:%M:%S", date: "$StatusUpdateTime" } },
    ...
    docs.forEach(doc => {
      if (doc.CallingUserID && typeof doc.CallingUserID !== 'string') doc.CallingUserID = doc.CallingUserID.toString();
      csvStream.write(doc);
    });
