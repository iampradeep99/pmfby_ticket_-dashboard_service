...
// import Ticket from "../../../../assets/ticketImg.jpeg";
// import SmsIcon from "../../../../assetsasync/messageImg.jpeg";
import * as moment from 'moment'
// import {stepsTATJourneyCrpLs,stepsTATJourneyGrv} from './pdf-generation-common'
import { MongoClient } from "mongodb";
var selectedData;
// const  Ticket = require('../../../../assets/ticketImg.jpeg')
...
//console.log('worker-runner: initialized');
export class PDFGenerationWorkerService {
  ...
  constructor() {
    ...
    //console.log('PDFGenerationWorkerService: Client initialized');
  }

  private initializeClient(): AxiosInstance {
    //console.log('PDFGenerationWorkerService: Initializing Axios client');
    return axios.create({
   ...
   async sendPDFToFarmer(payload:any){
    try{

      //console.log("Inside the farmer PDF generation method")
        selectedData = await this.fetchSelectedData(payload?.SupportTicketNo)
      let ticketListDetails:any = await this.FetchTicketInformation(payload)
      //console.log(ticketListDetails)
      let prinHTML = await this.renderPMFBYTemplate(selectedData,ticketListDetails)
      
      console.log(prinHTML, "htmlHTML")
    }catch(err){
      //console.log(err)
    }
  }
  async FetchTicketInformation(body: any): Promise<void> {
    //console.log('FetchTicketInformation: Started');
    if (!body) {
      //console.log('FetchTicketInformation: Request body is required');
      throw new Error("Request body is required");
    }
    if (!this.token) {
      //console.log('FetchTicketInformation: Missing token');
      throw new Error("Missing token");
    }

    const { SupportTicketNo } = body;
    if (!SupportTicketNo) {
      //console.log('FetchTicketInformation: SupportTicketNo is required');
    ...
    try {
      //console.log(`FetchTicketInformation: Sending request for ticket ${SupportTicketNo}`);
      const response = await this.sendRequest(payload);

      if (response?.data?.responseDynamic) {
        //console.log(`FetchTicketInformation: Received responseDynamic for ticket ${SupportTicketNo}`);
        ...
        //console.log(`FetchTicketInformation: Ticket details built for ticket ${SupportTicketNo}, total: ${TicketInfo.length}`);
        return TicketInfo
  ...
  private async sendRequest(payload: any) {
    //console.log('sendRequest: Sending POST request');
  ...
  private async decompressResponse(compressedData: string | Uint8Array): Promise<any> {
    //console.log('decompressResponse: Starting decompression');
    try {
      const dataBuffer = typeof compressedData === "string"
        ? Uint8Array.from(Buffer.from(compressedData, "base64"))
        : compressedData;

      const decompressed = pako.inflate(dataBuffer, { to: "string" });
      //console.log('decompressResponse: Decompression successful');
      return JSON.parse(decompressed);
    } catch (err) {
      //console.log(`decompressResponse: Decompression failed - ${err}`);
      throw err;
    }
  }

  private async TicketDetailsBuildUp(data: any) {
    //console.log('TicketDetailsBuildUp: Building ticket details');
    ...
    //console.log(`TicketDetailsBuildUp: Combined tickets count: ${combinedTickets.length}`);
  ...
  /*  private async getStatusWiseTemplate(pStatusID?:any, pData?:any, pselectedData?:any, pticketHistoryData?:any, pticketStatus?:any){
    try{
    ...
    rtnStatusWiseTemplate = [
      {
        ...
        date:
          pData && pData.CreatedAt
            ? await this.dateToSpecificFormat(`${pData.CreatedAt.split("T")[0]} ${await this.Convert24FourHourAndMinute(pData.CreatedAt.split("T")[1])}`, "DD-MM-YYYY HH:mm")
            : null,
        // <-- icon as HTML string (Ticket should be a string path/URL)
      ...
      },
      {
        ...
        date: "",
        // <-- replaced MdOutlineTextsms with HTML string (emoji). Replace with an SVG or <i> if you have icon font.
        ...
        date:
          pData && pData.TicketHistoryDate
            ? await this.dateToSpecificFormat(
                `${pData.TicketHistoryDate.split("T")[0]} ${await this.Convert24FourHourAndMinute(pData.TicketHistoryDate.split("T")[1])}`,
                "DD-MM-YYYY HH:mm",
              )
            : null,
        icon: `<img src="${"Ticket"}" width="24" height="24" />`,
  ...
  case 109303:
    ...
    if (pticketStatus === "Resolved") {
      pAgeiing = await this.getTicketAgeing(pselectedData.CreatedAt, pticketHistoryData, pselectedData.TicketHeaderID, pticketStatus);
    } else if (pticketStatus === "Resolved1" || pticketStatus === "Resolved2" || pticketStatus === "Resolved3") {
      const filterDataReOpen:any = Object.values(pticketHistoryData).find((entry:any) => entry.TicketStatus === pReOpenStatus);
      if (filterDataReOpen) {
        if (Object.values(filterDataReOpen).length > 0) {
          pAgeiing = await this.getTicketAgeing(filterDataReOpen.TicketHistoryDate, pticketHistoryData, selectedData.TicketHeaderID, pticketStatus);
        }
      }
    }
...
return rtnStatusWiseTemplate;
  ...
  //console.log(rtnStatusWiseTemplate, "rtnStatusWiseTemplate")
  ...
  try {
    ...
    // --------------------------------------------------------
    // 🔥 KEY FIX: Ensure ALL ASYNC fields resolve before return
    // --------------------------------------------------------
    const finalResolved = await Promise.all(
      rtnStatusWiseTemplate.map(async (item) => ({ ...item }))
    );
...
// --------------------------------------------------------
// Helper to correctly match ageing ranges (NO LOGIC CHANGE)
// --------------------------------------------------------
...
// --------------------------------------------------------
// MAIN FUNCTION WITH ONLY AGEING FIX ADDED
// --------------------------------------------------------
private async getStatusWiseTemplate(
  ...
  try {
    ...
    switch (pStatusID) {
      ...
      case 109303:
        ...
        // ---------------------------------------
        // FIXED AGEING RANGE MATCH (NO LOGIC CHANGE)
        // ---------------------------------------
        if (pselectedData.TicketHeaderID === 1) {
          for (let i = 0; i < stepsTATJourneyGrv.length; i++) {
    ...
    // Ensure all async values resolve
    const finalResolved = await Promise.all(
      rtnStatusWiseTemplate.map(async (item) => ({ ...item }))
    );
  ...
  private async dateToSpecificFormat(date, format = "DD-MM-YYYY HH:mm") {
...
private async  Convert24FourHourAndMinute(timeString = "") {
 ...
 async getTicketAgeing(createdDate:any, ticketHistory:any, ticketHeaderID:any, pResolvedStatus:any) {
    ...
    const resolvedEntry:any = Object.values(ticketHistory).find((entry:any) => entry?.TicketStatus === pResolvedStatus);
...
/*   async getCaseHistoryStepByStep(commentsTickets: any, pdata: any) {
  let rtnCommentsList: any[] = [];

  
  if (!Array.isArray(commentsTickets) || commentsTickets.length === 0) {
    return await this.getStatusWiseTemplate(
      pdata.TicketStatusID,
      pdata,
      "",
      ""
    );
  ...
  let allTicketStatusTemplate: any[] = [];

  const initialTemplate = await this.getStatusWiseTemplate(
    109301,
    pdata,
    "",
    ""
  ...
  if (Array.isArray(initialTemplate)) {
    allTicketStatusTemplate.push(...initialTemplate);
  }

  // PROCESS COMMENT ENTRIES SEQUENTIALLY
  for (const value of commentsTickets) {
    let statusTemplate: any = [];

    switch (value.TicketStatusID) {
      case 109302:   // In-Process
      case 109304:   // Re-Open
        statusTemplate = await this.getStatusWiseTemplate(
    ...
    // Only push valid non-empty templates
    if (Array.isArray(statusTemplate) && statusTemplate.length > 0) {
      allTicketStatusTemplate.push(...statusTemplate);
    }
  }

  return allTicketStatusTemplate;
} */
  ...
  async getCaseHistoryStepByStep(commentsTickets: any, pdata: any) {
    ...
    if (commentsTickets.length === 0) {
      ...
      if (commentsTickets.length > 0) {
        ...
        let statusTemplate = await this.getStatusWiseTemplate(109301, pdata, "", "");
        allTicketStatusTemplate.push(...statusTemplate);

        commentsTickets.forEach(async(value) => {
          if (value.TicketStatusID === 109302) {
            // A In-Process
            let statusTemplate = await this.getStatusWiseTemplate(value.TicketStatusID, value, pdata, "", "");
            allTicketStatusTemplate.push(...statusTemplate);
            ...
            // A Resolved
            let statusTemplate = await this.getStatusWiseTemplate(value.TicketStatusID, value, pdata, commentsTickets, value.TicketStatus);
            allTicketStatusTemplate.push(...statusTemplate);
          }
        });
...
async  renderPMFBYTemplate(selectedData: any = {}, ticketListDetails: any[] = [], helpers: any = {}) {
  ...
  // ---- Build ticketHtml from ticketListDetails ----
  let ticketHtml = ""
  ...
  for (let idx = 0; idx < (ticketListDetails || []).length; idx++) {
    ...
    // --- Build historiesHtml for this ticket ---
    let historiesHtml = ""
    ...
    if (Array.isArray(historiesArray)) {
      for (let i = 0; i < historiesArray.length; i++) {
        ...
        // Parse ticket description safely
        ...
        // UserType branch for agent id / user type
        let userTypeDisplay = ""
        if (v && v.UserType === "CSC") {
        ...
        // Build HTML block for this history item
        historiesHtml +=
          "" +
    ...
    // --- Build commentsHtml for this ticket ---
    let commentsHtml = ""
...
/*       if (Array.isArray(data?.Comments || []) && (data.Comments || []).length > 0) {
  ...
  if (Array.isArray(steps)) {
    for (let i = 0; i < steps.length; i++) {
      ...
      // SMS hover card
      let smsHoverHtml = ""
      if (step.smsText && String(step.smsText).trim() !== "") {
        smsHoverHtml =
          '<div class="hover-cardSendPdf"><p>' + step.smsText + "</p></div>"
      }

      // Decide classes based on TOP or NORMAL item
      const circleClass =
        i === 0
          ? "circleSendPdf circle-topSendPdf"
          : "circleSendPdf"

      const textClass =
        i === 0
          ? "text-partSendPdf text-topSendPdf"
          : "text-partSendPdf"

      const connectorClass =
        i === 0
          ? "connectorSendPdf connector-bottomSendPdf"
          : "connectorSendPdf"

      const detailsClass =
        i === 0
          ? "detailsSendPdf details-bottomSendPdf"
          : "detailsSendPdf"
    ...
    // --- Build the main ticket HTML block (left and right panels) ---
    // Format the CreatedAt date/time for the "Ticket Created" accordion
    ...
    // Status class fix: replace all '-' with '' and lowercase
    let statusClass = ""
    ...
    // Source and agent display fields
    ...
    ticketHtml +=
      ...
      "      </div>" +
      "    </div>" +
      // Append histories built earlier
      historiesHtml +
      "  </div>" +
      // Right panel with customer/info
  ...
  // ---- Build full HTML (same structure as original, cleaned) ----
  // Format createdAt of selectedData for header table
