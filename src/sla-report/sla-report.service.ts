  ...
  [
    {
      $addFields: {
        Call_Start_Time: {
          $cond: [
            { $eq: [{ $type: "$Call_Start_Time" }, "date"] },
            "$Call_Start_Time", // already Date
            {
              $cond: [
                {
                  $or: [
                    { $eq: ["$Call_Start_Time", "0000-00-00 00:00:00"] },
                    { $eq: ["$Call_Start_Time", ""] },
                    { $eq: ["$Call_Start_Time", null] }
                  ]
                },
                null,
                {
   ...
   async calculateCallQualityRecords(payload: any) {
    try {
      ...
      while (true) {
        ...
        console.log(JSON.stringify([...pipelineBase]))
