              ...
              $addFields: {
                ...
                Call_Start_Time: {
                  $cond: [
                    ...
                    {
                      $cond: [
                        { $in: ["$Call_Start_Time", ["0000-00-00 00:00:00", "", null]] },
                        null,
                        {
                          $dateFromString: {
                ...
                Call_End_Time: {
                  $cond: [
                    ...
                    {
                      $cond: [
                        { $in: ["$Call_End_Time", ["0000-00-00 00:00:00", "", null]] },
                        null,
                        {
                          $dateFromString: {
  ...
  try {
    ...
    const safeAggregate = async (collectionName, pipeline, stageNames) => {
      ...
      try {
        stageNames.forEach((name, index) => {
          this.logger.info(`⏳ Executing Stage ${index + 1}: "${name}" in "${collectionName}"`);
        });
        ...
        this.logger.info(`✅ Aggregation completed for "${collectionName}"`);
        return result || [];
      } catch (err) {
        this.logger.error(`❌ Aggregation failed for "${collectionName}":`, err);
        return [];
      }
    };
    ...
    const generateReportForDates = async (startOfMonth, endOfMonth) => {
      ...
      for (
        ...
        this.logger.info(`\n🔹 Processing date: ${currentFormattedDate}`);
        ...
        if (existingRecord) {
          this.logger.info(`⚠️ Record already exists for ${currentFormattedDate}, skipping.`);
          continue;
        }

        const inboundData = await safeAggregate(
          InboundCollectionName,
          [
            {
              $addFields: {
                Call_Start_Time: {
            ...
            {
              $match: {
                Call_Start_Time: { $gte: startOfDay, $lt: endOfDay },
                Status: { $ne: "System Missed" },
              },
            },
            { $count: "count" },
          ],
          ["Convert Call_Start_Time", "Filter by Date & Status", "Count Inbound"]
        );

        const outboundData = await safeAggregate(
          OutboundCollectionName,
          [
            {
              $addFields: {
                CUST_CALL_START_TIME: {
                  $cond: {
                    if: { $eq: [{ $type: "$CUST_CALL_START_TIME" }, "date"] },
                    then: "$CUST_CALL_START_TIME",
                    else: {
                      $cond: {
                        if: {
                          $in: ["$CUST_CALL_START_TIME", ["0000-00-00 00:00:00", "", null]],
                        },
                        then: null,
                        else: {
                          $dateFromString: {
            ...
            {
              $match: {
                CUST_CALL_START_TIME: { $gte: startOfDay, $lt: endOfDay },
                CUST_CALL_STATUS: { $ne: "System Missed" },
              },
            },
            { $count: "count" },
          ],
          ["Convert CUST_CALL_START_TIME", "Filter by Date & Status", "Count Outbound"]
        );

        // ================= ACTIVE AGENTS (UNIQUE) =================
        const agentData = await safeAggregate(
          AgentActivityCollectionName,
          [
            {
              $addFields: {
                tc_date_as_date: {
            ...
            {
              $match: {
                tc_date_as_date: { $gte: startOfDay, $lt: endOfDay },
              },
            },
            {
              $group: {
          ...
          ["Convert tc_date", "Filter by Date", "Group Unique Agents", "Count Agents"]
        );

        const totalInbound = inboundData?.length ? inboundData[0].count : 0;
        const totalOutbound = outboundData?.length ? outboundData[0].count : 0;
        const totalAgents = agentData?.length ? agentData[0].count : 0;
        ...
        try {
          await this.db.collection(targetCollection).insertOne(response);
          this.logger.info(
            `✅ Inserted ${currentFormattedDate} | Inbound: ${totalInbound} | Outbound: ${totalOutbound} | Agents: ${totalAgents}`
          );
          reports.push(response);
        } catch (insertErr) {
          this.logger.error(`❌ Insert failed for ${currentFormattedDate}:`, insertErr);
        }
      }
    ...
    if (reports.length > 0) {
      this.logger.info(
        `🎉 Completed for ${monthYear}. Inserted ${reports.length} new records.`
      );
    } else {
      this.logger.info(`⚠️ No new records generated for ${monthYear}`);
    }

    this.logger.info("🏁 Seat Utilization Process Completed Successfully");

    return reports;
  } catch (err) {
    this.logger.error("❌ Critical error during Seat Utilization:", err);
    throw err;
  }
}
