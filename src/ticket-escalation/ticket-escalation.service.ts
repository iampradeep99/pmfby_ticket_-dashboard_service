...
export class TicketEscalationService {
 ...
 /*  async RoleWiseAssignedTickets(payload: any) {
  ...
  try {
    ...
    const pipeline: any[] = [
      // Only get active assignments for the logged-in user
      { $match: { assignedTo: loggedInUserId, IsActive: true } },
      {
        $lookup: {
          from: "SLA_Ticket_listing",
      ...
      // Skip tickets that have already been reassigned
      {
        $addFields: {
          TicketRecords: {
            $filter: {
              input: "$TicketRecords",
              as: "ticket",
              cond: { $or: [{ $eq: ["$$ticket.AssignedTo", loggedInUserId] }, { $eq: ["$$ticket.AssignedTo", null] }] }
            }
          }
        }
      },
